...
function [Br, Bt, Ct, Dr, Dt, Et, alpha__r, alpha__t] = MF96_MZ0_coeffs(kappa, alpha, phi, Fz, tyre_data)
 ...
 % main code
  ...
  SHy = (dfz * pHy2 + pHy3 * gamma__z + pHy1) * LHY;
  SVy = Fz * (pVy1 + pVy2 * dfz + (dfz * pVy4 + pVy3) * gamma__z) * LVY * LMUY;
  Kya = FZ01 * pKy1 * sin(0.2e1 * atan((Fz / FZ01 / pKy2))) * (1 - pKy3 * abs(gamma__z)) * LFZ0 * LKA;
  SHf = SHy + SVy / Kya;
  ...
  Cy = pCy1 * LCY;
  mu__y = (dfz * pDy2 + pDy1) * (-pDy3 * gamma__z ^ 2 + 1) * LMUY;
  Dy = mu__y * Fz;
  Kya = FZ01 * pKy1 * sin(0.2e1 * atan((Fz / FZ01 / pKy2))) * (1 - pKy3 * abs(gamma__z)) * LFZ0 * LKA;
  By = Kya / Cy / Dy;
